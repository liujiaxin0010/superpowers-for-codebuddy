...
## 查询流程契约：模糊 → Boss 确认 → 精确（强制三阶段）

> **为什么要三阶段**：模式 A/D（精确调用链）对**目标函数的精度**敏感——如果目标函数/文件猜错，后续调用链追踪的 blast radius 就是基于错误起点的结论。
> 必须先用模式 F（模糊匹配）锁定候选，Boss 核实后，再做深度精确分析，否则"高置信度错误调用链"比"无调用链"更危险。

### 阶段 1：模糊定位（code-explorer / 模式 F）

**触发**：任何"从业务描述找代码"的任务。

**动作**：
1. 调用 GitNexus `search`（混合 BM25 + 语义 + 图扩展）
2. 返回 Top 3-5 候选业务函数/文件，每个候选包含：
   - `file:line` + 函数名
   - 一句话职责描述（AI 翻译为业务语义）
   - 匹配置信度（GitNexus 评分）

**约束**：
- 阶段 1 **不得直接**进入模式 A/D 做深度分析
- 阶段 1 **不得跳过** `search` 而直接 grep——会丢失图扩展维度

### 阶段 2：Boss 核实（硬门禁）

**触发**：阶段 1 返回的候选 **> 1 个** 时强制。

**动作**：
1. 向 Boss 展示候选清单（表格 / 列表形式）
2. 请求 Boss 打勾选定目标
3. Boss 确认后在 `docs/progress.md` 记录：
   - 候选清单（含置信度）
   - 选定目标
   - 排除其他候选的原因

**例外**（允许跳过阶段 2，但必须声明）：
- 阶段 1 仅返回 1 个候选且置信度 ≥ 0.9
- 用户请求时已明确提供精确的 `file:line + 函数名`（直接进阶段 3）

**禁止**：
- ❌ 候选 > 1 个却不问 Boss 直接挑一个做 A/D
- ❌ 在 `docs/progress.md` 不记录跳过原因
...
**动作**：
1. **模式 A**（文件 360° 上下文）：目标文件的 INPUT/OUTPUT/POS/Community
2. **模式 D**（调用链追踪）：目标函数的上下游（callers / callees）
3. **模式 C**（变更影响分析）：本次变更的 blast radius（用于 `/write-plan` 估算）
4. 可选 **模式 B**（模块全景）：需要扩展到整个模块时追加

### 降级路径

GitNexus 不可用时，三阶段结构仍保留：
- 阶段 1 → `grep` 语义关键词搜索 / 文件名模糊匹配
- 阶段 2 → 仍需 Boss 核实候选
- 阶段 3 → 手动阅读目标文件及其 import 链

降级在 `docs/progress.md` 记录原因，不询问 Boss。
