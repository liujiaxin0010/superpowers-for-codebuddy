请按以下顺序阅读并严格遵循：
1. `.codebuddy/skills/ai-interaction-scoring/SKILL.md`（评分维度与规则）
2. `.codebuddy/skills/ai-interaction-scoring/scoring-rules.json`（评分配置）
3. `.codebuddy/skills/ai-interaction-scoring/templates/score-report-template.md`（报告模板）
4. `.codebuddy/skills/xlsx/SKILL.md`（XLSX 输出）

你的任务是：
评估一段 AI 编码助手的对话，生成结构化的质量评分报告。

执行步骤：
1. 解析参数：`name=<姓名>`、`id=<工号>`、`task=<需求名称>`、`projectDir=<项目路径>`、`outputDir=<输出路径>`
2. **如果缺少姓名、工号或需求名称，必须先向用户询问，不得跳过**
3. 接收对话内容（粘贴文本、文件路径或当前对话上下文）
4. 识别任务类型，确定 5 个与任务相关的核心交付物
5. 在 4 个评分维度中收集证据：
   - **维度一（Spec-Coding 规范，10 分）**：检查需求澄清、方向评估、规格文档、实施计划、持久化文档、门禁检查
   - **维度二（Skills/Agent 使用，5 分）**：统计不同技能/Agent 的调用次数（去重）
   - **维度三（项目完成度，10 分）**：按证据级别（完整/部分/无）评估每个交付物。用户反馈的故障优先于代码证据
   - **维度四（扩展功能与美化，5 分）**：检查是否有超出基本需求的功能和界面美化
6. **代码分析（可选）**：若提供了 `projectDir`，读取 `references/code-analysis-guide.md`，对每个交付物执行：
   - 文件存在性检查：对话中声称的文件是否在项目中真实存在
   - 实现充分性检查：关键函数/组件是否有实际实现而非空壳
   - 测试覆盖检查：测试文件和用例是否存在
   - 结构/依赖/构建完整性检查
   - 将代码分析结果与对话证据交叉验证，确定最终证据级别
7. 检测反模式：虚假完成、过早编码、Bug 修复循环、忽略用户反馈、代码虚报（需启用代码分析）
8. 计算各维度得分及总分
9. 识别亮点（优势与不足）
10. 生成改进建议
11. 使用模板输出 `{姓名}_{工号}_{需求名称}.md`
12. 使用 xlsx 技能输出 `{姓名}_{工号}_{需求名称}.xlsx`

文件命名规则：
- 报告文件名格式：`{姓名}_{工号}_{需求名称}.md` / `.xlsx`
- 示例：`张三_10086_文件映射服务.md`、`张三_10086_文件映射服务.xlsx`
- 姓名、工号、需求名称三个字段缺一不可，缺失时必须先询问用户

评分约束：
- 基于**对话中的证据**评分，而非假设
- 证据模糊时，采用保守评分（偏低）
- 用户关于功能的陈述优先于代码分析
- 编译成功**不**足以作为"功能正常"的充分证据
- 各维度独立评分
- 报告中每个得分必须有可追溯的证据引用
- 反模式即使不影响评分也必须标记

输出要求：
- 报告语言与对话语言一致（中文对话 = 中文报告）
- XLSX 内容使用中文表头和描述
- 所有得分必须有可追溯的证据
- 改进建议必须具体、可操作

$ARGUMENTS
