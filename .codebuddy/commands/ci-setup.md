...
3. **采集参数**（一次问清，不要逐项追问）：
   - 内网 Docker registry 镜像地址（含 bash + git，用于替换 `<PLACEHOLDER:INTERNAL_REGISTRY_IMAGE>`）
   - 主分支名（`master` 或 `main`）
   - Runner 类型（Linux/bash 默认，或 Windows/pwsh）
   - 是否强制 commit message 工单号（默认否）

4. **生成产物**（实例化 `ci-integration/templates/` 下模板到业务项目）：
   - `.gitlab-ci.yml` ← `gitlab-ci.yml.template`，替换占位符；Windows runner 则把 `bash *.sh` 改为 `pwsh -File *.ps1`
   - `.gitlab/merge_request_templates/featureflow.md` ← `merge_request_template.md`
   - `scripts/commit-msg-lint.sh`（及 `.ps1`）← 对应模板；若强制工单号，在脚本注释指引处启用 `REQUIRE_ISSUE_REF`
...
- `<PLACEHOLDER:INTERNAL_REGISTRY_IMAGE>` 必须替换，不得留在产物中
