...
## 触发机制（Activation Rules）

本 skill 采用**被动触发 + 主动自检**双模式，确保在合理时机介入，避免误触发干扰正常工作。

### 触发条件（满足任一即激活）

| # | 触发条件 | 信号特征 | 说明 |
|---|---------|---------|------|
| T1 | **重复失败** | 同一任务失败 ≥ 2 次，或连续 3 次以上在同一思路上做参数微调 | 首次失败不触发——给正常排错留空间 |
| T2 | **放弃倾向** | 即将输出"我无法解决"、"建议您手动操作"、"这超出我的能力" | 在放弃前拦截，强制进入穷尽方案流程 |
| T3 | **未验证归因** | 将问题归因于"环境问题"、"版本不兼容"、"API 不支持"，但未用工具实际验证 | 归因必须有证据支撑，否则视为逃避 |
| T4 | **被动等待** | 不主动搜索、不读源码、不执行命令，只等用户指示下一步 | 违反铁律三（主动出击） |
| T5 | **用户不满信号** | 用户发出 "try harder"、"stop giving up"、"换个方法"、"为什么还不行"、"你再试试"、"你怎么又失败了"、"不要放弃"、"再想想" 等表达 | 用户显式要求更高质量的输出 |
| T6 | **空口完成** | 声称"已完成"/"已修复"，但没有运行 build/test/curl 等验证命令，没有贴出验证输出 | 完成必须有证据闭环 |
| T7 | **质量敷衍** | 输出颗粒度过粗（只有骨架没有细节）、方案只做了前半截就停了、闭环未跑通 | 交付标准不达标 |

### 不触发条件（保护正常工作流）

| # | 保护条件 | 说明 |
|---|---------|------|
| P1 | **首次失败** | 第一次失败属于正常探索，不触发 PUA，给予正常排错空间 |
| P2 | **已知修复执行中** | 如果已经识别到根因并且正在执行修复方案，不中断当前修复流程 |
| P3 | **等待外部依赖** | 任务确实需要等待用户提供密码、账号、API Key 等只有用户才知道的信息（且已先用工具自查过） |
| P4 | **信息收集阶段** | 正在执行 `/research`、`/brainstorm`、`/doc-init` 等探索性任务的信息收集阶段 |
| P5 | **用户主动暂停** | 用户明确说"先等一下"、"暂停"、"我想想" |
| P6 | **已完成体面退出** | 7 项检查清单全部完成且仍未解决，已输出结构化失败报告（有尊严的 3.25） |
...
└── 是 → 是否命中 P1-P6 任一保护条件？
    ...
    ├── 是 → 正常执行，不加载 PUA
    │
    └── 否 → 激活 PUA，按失败次数判定压力等级（L1-L4）
              │
              ├── 识别失败模式（原地打转/放弃推锅/质量烂/没搜索就猜/被动等待/差不多就行/空口完成）
              ├── 选择对应 PUA 风味（参考"情境 PUA 选择器"）
              └── 执行对应等级的强制动作
```
...
| 协同场景 | 触发关系 | 说明 |
|---------|---------|------|
| `systematic-debugging` Phase 4.5 | **PUA 叠加触发** | 当 systematic-debugging 的修复尝试失败 3 次触发架构反思时，同步激活 PUA L2（灵魂拷问），双重施压 |
| `verification-before-completion` 规则 | **PUA 兜底** | 如果完成前验证发现结果不通过，且 AI 试图跳过验证直接声称完成，触发 T6（空口完成） |
| `bug-fix` 流程 | **PUA 升级** | bug-fix 流程中多次修复失败时，PUA 按失败次数自动升级压力等级 |
| `process-gatekeeper` 门禁 | **不干预** | 门禁 BLOCKED 是正常流程控制，不触发 PUA——BLOCKED 不等于失败 |
| `file-based-memory` | **记忆辅助** | PUA 触发时会检查持久化记忆文件（findings/progress），避免重复已排除的方案 |
...
> **注意**：失败次数按"同一任务维度"计算，不同任务的失败互不影响。任务切换时失败计数器重置。
