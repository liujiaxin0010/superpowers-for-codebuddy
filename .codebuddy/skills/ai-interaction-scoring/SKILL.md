...
- `templates/score-report-template.md`
...
只在需要判断”哪些证据能算分、哪些证据应保守处理”时，再读取：

- `references/evidence-patterns.md`

### 代码分析时

仅当提供了 `projectDir` 参数时，再读取：

- `references/code-analysis-guide.md`

### 不要怎么加载

1. 不要一开始就把模板和规则文件都全量展开后再分析对话
2. 先做证据抽取，再加载模板写报告
3. 不要在未提供 `projectDir` 时加载代码分析指南
...
4. **代码分析**（可选）：若提供了 `projectDir`，读取 `references/code-analysis-guide.md`，对每个交付物执行文件存在性、实现充分性、测试覆盖等检查
5. **交叉验证**（可选）：将代码分析结果与对话证据交叉比对，按规则确定最终证据级别
6. **分数计算**：按评分规则映射证据并计算得分
7. **亮点分析**：识别优势和不足
8. **报告生成**：输出 MD 报告 + XLSX 汇总表
