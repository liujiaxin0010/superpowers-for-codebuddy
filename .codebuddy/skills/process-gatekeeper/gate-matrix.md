...
| `/code-review` | 可选，但若执行必须输出风险声明与证据完整性 | M/H 必需，且应关联 `spec/plan` 与 owner/handoff 信息 | `/code-review ...` |
...
| `/devflow-ai` | 需先识别任务类型并给出推荐命令 | 必须输出缺失前置与下一步动作，不得跳过路由直接乱执行 | `/devflow-ai ...` |
