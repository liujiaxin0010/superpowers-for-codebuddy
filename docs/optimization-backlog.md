...
| OPT-R1 | ★★★ | 评审按 diff 风险加权 + 安全/性能基于 diff 自动触发，而非读 spec 关键词 |
...
| 编号 | 问题 | 优化 | 杠杆 | 依赖 |
|---|---|---|---|---|
| OPT-R1 | 审查全量、安全/性能靠读 spec 关键词触发（可能漏触发） | 按 diff 风险加权审查深度；碰 crypto/auth/支付/数据层等路径**基于 diff 自动**强制 security/perf | ★★★ | — |
